...
def handle_get_career_background_verification_closure_evidence_snapshot_comparison(command):
    application_id = command[
        len("career background verification closure evidence snapshot comparison "):
    ].strip()

    if not application_id.isdigit():
        return (
            "Usage: career background verification closure evidence snapshot comparison "
            "<application_id>"
        )

    application_id = int(application_id)

    return get_career_background_verification_closure_evidence_snapshot_comparison(
        application_id
    )


def handle_get_career_background_verification_closure_evidence_snapshot_trend(command):
    application_id = command[
        len("career background verification closure evidence snapshot trend "):
    ].strip()

    if not application_id.isdigit():
        return (
            "Usage: career background verification closure evidence snapshot trend "
            "<application_id>"
        )

    application_id = int(application_id)

    return get_career_background_verification_closure_evidence_snapshot_trend(
        application_id
    )


def handle_get_career_background_verification_closure_evidence_snapshot_forecast(command):
    application_id = command[
        len("career background verification closure evidence snapshot forecast "):
    ].strip()

    if not application_id.isdigit():
        return (
            "Usage: career background verification closure evidence snapshot forecast "
            "<application_id>"
        )

    application_id = int(application_id)

    return get_career_background_verification_closure_evidence_snapshot_forecast(
        application_id
    )



def handle_get_career_background_verification_closure_evidence_snapshot_prediction(command):
    application_id = command[
        len("career background verification closure evidence snapshot prediction "):
    ].strip()

    if not application_id.isdigit():
        return (
            "Usage: career background verification closure evidence snapshot prediction "
            "<application_id>"
        )
