...
def get_career_experience_certificate_request(application_id):
    application = get_job_application(application_id)
    ...
    subject = f"Request for Experience Certificate - {company}"

    message = (
        "Dear HR,\n\n"
        f"I am writing to request an experience certificate for my role as "
        f"{role} at {company}.\n\n"
        "I would be grateful if the certificate could include my designation, "
        "employment period, and a brief confirmation of my work experience "
        "with the organization.\n\n"
        "Please let me know if any additional information, clearance, or "
        "documentation is required from my side to process the request.\n\n"
        "Thank you for your support and assistance.\n\n"
        "Best regards,\n"
        "Candidate"
    )
    ...
    return (
        f"JERVIS Career Experience Certificate Request Generator - Application "
        f"{application_id}\n"
        "--------------------------------------------------------------------\n"
        f"Company: {company}\n"
        f"Role: {role}\n"
        f"Subject: {subject}\n"
        "Generated Experience Certificate Request:\n"
        f"{message}\n"
        "Next Action: Review the message, replace 'Candidate' with your name, "
...
def get_career_background_verification_closure_action_plan(application_id):
    application = get_job_application(application_id)
    ...
    if current_status == "Closed" and reopened_count == 0:
        priority = "Low"
        action = (
            "Keep the closure confirmation and continue monitoring "
            "onboarding or joining requirements."
        )
        next_step = (
            "Monitor the remaining onboarding process."
        )
        ...
        priority = "Medium"
        action = (
            "Review previous reopening events and keep all verification "
            "records available."
        )
        next_step = (
            "Confirm that no additional verification requirement remains."
        )
    elif current_status == "Reopened":
        priority = "High"
        action = (
            "Review the reason for reopening and provide any requested "
            "information or documents."
        )
        next_step = (
            "Resolve the reopened verification requirement."
        )
    elif current_status == "Pending Confirmation":
        priority = "Medium"
        action = (
            "Request final confirmation from the background verification "
            "or HR team."
        )
        next_step = (
            "Obtain confirmation before treating the case as closed."
        )
    else:
        priority = "Medium"
        action = (
        ...
        next_step = (
            "Follow up with the verification team if the case remains open."
        )

    return (
        f"JERVIS Career Background Verification Closure Action Plan "
        f"- Application {application_id}\n"
        "------------------------------------------------------------\n"
        f"Company: {company}\n"
        f"Role: {role}\n"
...
def analyze_career_background_verification_closure_evidence_retention(application_id):
    application = get_job_application(application_id)
    ...
    for index, event in enumerate(history, start=1):
        if not event.get("from_status") or not event.get("to_status") or not event.get("changed_at"):
            retention_score -= 15
            concerns.append(f"Audit event {index} has incomplete retention evidence.")

        if index > 1 and event.get("from_status") != history[index - 2].get("to_status"):
            retention_score -= 15
            concerns.append(f"Audit event {index} breaks retention continuity.")
    ...
    if retention_score >= 90:
        retention_status = "RETENTION READY"
    elif retention_score >= 70:
        retention_status = "REVIEW REQUIRED"
    else:
        retention_status = "NOT READY"

    if retention_status == "RETENTION READY":
        next_action = "Retain the closure evidence as a retention-ready record."
    elif retention_status == "REVIEW REQUIRED":
        next_action = "Review the retention concerns before long-term retention."
    else:
        next_action = "Resolve the retention concerns before long-term retention."

    concern_summary = "; ".join(concerns) if concerns else "No retention concerns detected."

    return (
        f"JERVIS Career Background Verification Closure Evidence Retention Analyzer - Application {application_id}\n"
        "------------------------------------------------------------\n"
        f"Company: {company}\n"
        f"Role: {role}\n"
        f"Closure Status: {closure_status}\n"
        f"Audit Event Count: {len(history)}\n"
        f"Retention Score: {retention_score}/100\n"
        f"Retention Status: {retention_status}\n"
        f"Concerns: {concern_summary}\n"
        f"Next Action: {next_action}"
    )
...
def get_career_background_verification_closure_evidence_snapshot_prediction(application_id):
    application = get_job_application(application_id)
    ...
    if len(snapshot_history) < 2:
        return (
            f"JERVIS Career Background Verification Closure Evidence Snapshot Prediction - Application {application_id}\n"
            "------------------------------------------------------------\n"
            f"Company: {company}\n"
            f"Role: {role}\n"
            f"Snapshot Count: {len(snapshot_history)}\n"
            "At least 2 evidence snapshots are required for prediction analysis."
        )
    ...
    if latest_score > previous_score:
        ...
        next_action = "Maintain progress and capture the next evidence update."
    elif latest_score < previous_score:
        prediction = "REGRESSION"
        next_action = "Investigate the latest regression and restore evidence readiness."
    else:
        prediction = "STABLE"
        next_action = "Monitor the next snapshot for a meaningful status change."
