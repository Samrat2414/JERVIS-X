...
def test_career_employment_verification_request_not_found():
    result = get_career_employment_verification_request("999")

    assert result == "Job application not found."


def test_career_employment_verification_request_header():
    add_test_application()

    result = get_career_employment_verification_request("1")

    assert "JERVIS Career Employment Verification Request Generator" in result
    assert "Application 1" in result


def test_career_employment_verification_request_company_and_role():
    add_test_application()
...
def test_career_employment_verification_request_subject():
    add_test_application()

    result = get_career_employment_verification_request("1")

    assert (
        "Subject: Request for Employment Verification - Test Company"
        in result
    )


def test_career_employment_verification_request_message_content():
    add_test_application()

    result = get_career_employment_verification_request("1")

    assert "request employment verification" in result
    assert "designation" in result
    assert "employment period" in result
    assert "employment status" in result


def test_career_employment_verification_request_background_check():
    add_test_application()

    result = get_career_employment_verification_request("1")

    assert "background verification purposes" in result
    assert "additional authorization" in result
    assert "documentation" in result


def test_career_employment_verification_request_signature():
    add_test_application()
...
def test_career_employment_verification_request_next_action():
    add_test_application()
...
def test_career_background_verification_closure_evidence_compliance_ready():
    add_test_application()
    ...
    result = analyze_career_background_verification_closure_evidence_compliance("1")

    assert "Compliance Score: 100/100" in result
    assert "Compliance Status: COMPLIANT" in result
    assert "No compliance concerns detected." in result


def test_career_background_verification_closure_evidence_compliance_not_found():
    result = analyze_career_background_verification_closure_evidence_compliance("999")

    assert result == "Job application not found."


def test_career_background_verification_closure_evidence_compliance_open():
    add_test_application()

    result = analyze_career_background_verification_closure_evidence_compliance("1")

    assert "Closure Status: Open" in result
    assert "Compliance Score: 20/100" in result
    assert "Compliance Status: NON-COMPLIANT" in result
    assert "Background verification closure is not Closed." in result
    assert "No closure audit history is available." in result



def test_career_background_verification_closure_evidence_retention_ready():
    add_test_application()
    ...
    result = analyze_career_background_verification_closure_evidence_retention("1")

    assert "Retention Score: 100/100" in result
    assert "Retention Status: RETENTION READY" in result
    assert "No retention concerns detected." in result


def test_career_background_verification_closure_evidence_retention_not_found():
    result = analyze_career_background_verification_closure_evidence_retention("999")

    assert result == "Job application not found."


def test_career_background_verification_closure_evidence_retention_open():
    add_test_application()

    result = analyze_career_background_verification_closure_evidence_retention("1")

    assert "Closure Status: Open" in result
    assert "Retention Score: 20/100" in result
    assert "Retention Status: NOT READY" in result
    assert "Background verification closure is not Closed." in result
    assert "No closure audit history is available." in result
...
def test_career_background_verification_closure_evidence_snapshot_forecast_not_found():
    result = get_career_background_verification_closure_evidence_snapshot_forecast("999")
    assert result == "Job application not found."

def test_career_background_verification_closure_evidence_snapshot_forecast_insufficient_snapshots():
    add_test_application()
    result = get_career_background_verification_closure_evidence_snapshot_forecast("1")
    assert "Snapshot Count: 0" in result
    assert "At least 2 evidence snapshots are required for forecast analysis." in result

def test_career_background_verification_closure_evidence_snapshot_forecast_watch():
    add_test_application()
    record_career_background_verification_closure_evidence_snapshot("1")
    record_career_background_verification_closure_evidence_snapshot("1")
    result = get_career_background_verification_closure_evidence_snapshot_forecast("1")
    assert "Forecast: WATCH" in result
    assert "Score Direction: 0 -> 0" in result
    assert "Next Action: Monitor for new evidence or status changes." in result

def test_career_background_verification_closure_evidence_snapshot_forecast_positive():
    add_test_application()
    record_career_background_verification_closure_evidence_snapshot("1")
    update_career_background_verification_status("1", "Submitted")
    record_career_background_verification_closure_evidence_snapshot("1")
    result = get_career_background_verification_closure_evidence_snapshot_forecast("1")
    assert "Forecast: POSITIVE" in result
    assert "Score Direction: 0 -> 1" in result
    assert "Next Action: Continue current verification and closure progress." in result

def test_career_background_verification_closure_evidence_snapshot_forecast_negative():
    add_test_application()
    update_career_background_verification_status("1", "Submitted")
    record_career_background_verification_closure_evidence_snapshot("1")
    update_career_background_verification_status("1", "Not Submitted")
